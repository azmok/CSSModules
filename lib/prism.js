/* PrismJS 1.29.0
https://prismjs.com/download.html#themes=prism-okaidia&languages=markup+css+clike+javascript+abnf+clojure+ebnf+git+http+json+markdown+markup-templating+mongodb+php+python+jsx+sass+scss+scheme+web-idl+yaml&plugins=line-highlight+line-numbers+normalize-whitespace+toolbar+copy-to-clipboard+bash */
...
Prism.languages.clike = {
   ...
   'keyword-crucial': [ // keywords wanting to highlight with red
		{
			pattern: /((?:^|\})\s*)catch\b/,
			lookbehind: true
		},
		{
			pattern: /(^|[^.]|\.\.\.\s*)\b(?:as|async(?=\s*(?:function\b|\(|[$\w\xA0-\uFFFF]|$))|await|break|case|continue|debugger|default|delete|do|else|enum|export|extends|for|from(?=\s*(?:['"]|$))|(?:get|set)(?=\s*(?:[#\[$\w\xA0-\uFFFF]|$))|if|implements|import|in|instanceof|interface|new|of|package|private|protected|return|switch|throw|try|typeof|while|void|with|yield)\b/,
			lookbehind: true
		}
   ],
   'keyword-primitives': /\b(?:undefined|null)\b/,
...
Prism.languages.javascript = Prism.languages.extend('clike', {
	...
	'keyword-normal': [
		{
			pattern: /(^|[^.]|\.\.\.\s*)\b(assert(?=\s*\{)|class|const|finally(?=\s*(?:\{|$))|function|(?:get|set)(?=\s*(?:[#\[$\w\xA0-\uFFFF]|$))|let|public|static|super|this|var)\b/,
			lookbehind: true
		},
	],
...
Prism.languages.insertBefore('javascript', 'keyword-normal', {
	'regex': {
		pattern: RegExp(
			// lookbehind
...
//######  Prism.plugin.copyTextToClipboard
// line 4776 - 4935
(function () {
